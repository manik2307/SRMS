...
<nav class="navbar navbar-expand-lg navbar-light">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <li class="nav-item">
                <a class="nav-link" href="updateresult.php">Reload This Page</a>
            </li>
            <li class="nav-item">
    ...
    <h2>
      ...
      <label for="section">Enter Student Id:</label>
        ...
        <label for="section">Change Student Id to:</label>
        <input type="number" class="form-control" placeholder="1910990320"required="required" name="NewId">

        <label for="section">Change Class Id to:</label>
        <input type="number" class="form-control" placeholder="Class Id"required="required" name="NewClassId">

        <label for="section">Change Subject Id to:</label>
        <input type="number" class="form-control" placeholder="subject Id"required="required" name="NewSubjectId">

        <label for="section">Change marks to:</label>
        <input type="number" class="form-control" placeholder=" between 1 to 100"required="required" max = 100 name="NewMarks">
    ...
    <?php
        ...
        $new_student_id = $_GET['NewId'];
        $new_class_id = $_GET['NewClassId'];
        $new_subject_id = $_GET['NewSubjectId'];
        $new_marks = $_GET['NewMarks'];
        

        $sameSql = "SELECT * FROM `results` WHERE `StudentId` = '$new_student_id' AND `ClassId` = '$new_class_id' AND `SubjectId` = '$new_subject_id' AND `Marks` = '$new_marks'";
        ...
        $addstudentSql = "UPDATE `results` SET `StudentId` = '$new_student_id', `ClassId` = '$new_class_id', `SubjectId` = '$new_subject_id', `Marks` = '$new_marks' WHERE `StudentId` = '$student_id' AND `ClassId` = '$class_id' AND `SubjectId` = '$subject_id' AND `Marks` = '$marks' ";
        $studentAddResult = mysqli_query($conn, $addstudentSql);
        
        if($studentAddResult){
            echo "<script>
            alert('Added Successfully');
            window.location.href='http://localhost/SRMS/updateresult.php';
            </script>";
        }
        else {
