...
<nav class="navbar navbar-expand-lg navbar-light">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <li class="nav-item">
                <a class="nav-link" href="updatesubject.php">Reload This Page</a>
            </li>
            <li class="nav-item">
    ...
    <h2>
        Fill the info below to update subject:
    </h2>
    
                                                    
        <form action="updatesubject.php" method="get">
      ...
      <div class="alert alert-warning" role="alert">
            Keep The New Subject ID Same as old one. Contact Admin to Change Subject ID.
      </div>

      <div>
        <label for="className">New Subject Id:</label>
        <input type="number" class="form-control" placeholder="1,2,3,4,5...."required="required" name="NewId">
        <label for="section">New Subject Name:</label>
        <input type="text" class="form-control" placeholder="physics, evs....etc"required="required" name="NewName">
      </div>
    ...
    <?php
        ...
        $subject_id = $_GET['Id'];
        $new_subject_name = $_GET['NewName'];
        $new_subject_id = $_GET['NewId'];

        $sameSql = "SELECT * FROM `subjects` WHERE `SubjectId` = '$new_subject_id' AND `SubjectName` LIKE '$new_subject_name' ORDER BY `SubjectId` ASC";
        ...
        $add_subject_sql = "UPDATE `subjects` SET `SubjectId` = '$new_subject_id', `SubjectName` = '$new_subject_name' WHERE `subjects`.`SubjectId` = '$subject_id'";
        $add_subject_result = mysqli_query($conn, $add_subject_sql);

        if($add_subject_result){
            echo "<script>
                        alert('updated Successfully');
                        window.location.href='http://localhost/SRMS/updatesubject.php';
                </script>";
        }
        else {
